...
public class PDFService {
    ...
    //tải chi tiết bài thi của sv ra file PDF
    public void exportAttemptDetailPDF(AttemptViewDTO questionsDetail, QuizzesDTO.QuizzesListDTO quiz, List<QuestionDetail> questionDetails, String filePath, CoursesDTO coursesDTO, String exportDate) {
        try {
            ...
            Paragraph title = new Paragraph("Khóa học: " + coursesDTO.getFullname(), new Font(vietnameseFont, 18, Font.BOLD));
            ...
            addTableCell(table, "Tên kỳ thi", quiz.getName(), boldFont, nomalFont);
            ...
            addTableCell(table, "Ngày xuất bài thi", exportDate, boldFont, nomalFont);
...
//        cellHeader.setHorizontalAlignment(Element.ALIGN_RIGHT);
    ...
    public String sanitizeFileName(String name) {
        ...
        return noDiacritics
                .replaceAll("[\\\\/:*?\"<>|]", "") // Xóa các ký tự không hợp lệ cho tên file
                .replaceAll("\\s+", ""); // Xóa toàn bộ khoảng trắng
    }

    //tải chi tiết bài thi của gv - đề thi mẫu ra file PDF
    public void exportSampleAttemptPDF(AttemptViewDTO questionsDetail, QuizzesDTO.QuizzesListDTO quiz, List<QuestionDetail> questionDetails, String filePath, CoursesDTO coursesDTO, String exportDate) {
        try {
            Document document = new Document();
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(filePath));
            writer.setPageEvent(new PageNumberHelper());
            ...
            addTableCell(table, "Tên kỳ thi", quiz.getName(), boldFont, nomalFont);
            addTableCell(table, "Ngày xuất", exportDate, boldFont, nomalFont);
            addTableCell(table, "Thang điểm", quiz.getFormattedGrade(), boldFont, nomalFont);
